...
// キーパー動作用関数
// ==========================================
void chase_ball(double gyro_val){
    float ball_coefficient = 1.0; //ボールを追う強さ
    float line_coefficient = 5.0; //ラインを避ける強さ
    float ir_rad = ir_angle * PI / 180.0; //ラジアンに変換
    
    float b_x = cos(ir_rad) * ball_coefficient; //ボールのx成分(前方向ベクトル)
    float b_y = sin(ir_rad) * ball_coefficient; //ボールのy成分(横方向ベクトル)

    float temp_l_x = get_line_x(); //ラインのy成分(正面方向ベクトル)
    float l_x = (1 + temp_l_x) * line_coefficient; //ラインに触れたらとにかく前に避けるためのx成分調整
    float move_angle_rad = atan2(b_y , l_x); //最終的な移動角度計算
    float move_angle = move_angle_rad * 180.0 / PI;

    MotorDrive(move_angle, 80, gyro_val);
  ...
  if (game_flag != 0){
    ...
    if (game_flag == 2){
      if (line_flag == 1 && 205 < line_angle && line_angle < 245){
        MotorDrive(90, 70, (int)gyro_val); // ライン避ける
        x_position = 2;
      }
      if (line_flag == 1 && 115 < line_angle && line_angle < 155){
        MotorDrive(270, 70, (int)gyro_val); // ライン避ける
        x_position = 1;
      }
